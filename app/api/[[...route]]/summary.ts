...
const app = new Hono().get(
  ...
  async (c) => {
    ...
    const prevPeriodStart = subDays(startDate, periodLength);
    const prevPeriodEnd = subDays(endDate, periodLength);
    ...
    const [prevPeriod] = await fetchFinancialData(
      auth.userId,
      prevPeriodStart,
      prevPeriodEnd
    );
    ...
    const remainingChange = calculatePercentageChange(
      currPeriod.remaining,
      prevPeriod.remaining
    );
    ...
    return c.json({
      data: {
        ...
        incomeChange,
        expenseAmount: currPeriod.expense,
        expenseChange,
