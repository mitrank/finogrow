...
export const DataTableFacetedFilter = <TData, TValue>({
  ...
  const handleOnItemSelect = (isSelected: boolean, value: string) => {
    if (isSelected) {
      selectedValues.delete(value);
    } else {
      selectedValues.add(value);
    }
    const filteredValues = Array.from(selectedValues);
    column?.setFilterValue(filteredValues.length ? filteredValues : undefined);
  };

  return (
    <Popover>
      <PopoverTrigger asChild className="w-full lg:w-min">
        <Button>
          <PlusCircle />
          {title}
              ...
              {options.map((option) => {
                ...
                return (
                  <CommandItem
                    key={option.value}
                    onSelect={() =>
                      handleOnItemSelect(isSelected, option.value)
                    }
                  >
                    <div
                      className={cn(
